namespace SprintCrowd.BackEnd.SprintManager.Web
{
    ...
    public class SprintManagerController : ControllerBase
    {
        ...
        {
            await this.SprintParticipantService.UpdateParticipantStatus(race.UserId, race.SprintId, race.Time, ParticipantStage.COMPLETED, race.Position, race.RaceCompletedDuration, race.Distance);
        ...
        {
            await this.SprintParticipantService.UpdateParticipantStatus(race.UserId, race.SprintId, race.Time, ParticipantStage.QUIT, 0, string.Empty,race.Distance);
            return this.Ok();
        }
